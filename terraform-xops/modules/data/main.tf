...
resource "aws_secretsmanager_secret" "app_runtime" {
  name        = "${var.project_path}/app-runtime"
...
resource "aws_secretsmanager_secret" "mongodb_atlas" {
  ...
  name        = "${var.project_path}/mongodb-atlas-source"
...
resource "aws_secretsmanager_secret" "documentdb_placeholder" {
  count       = var.enable_documentdb ? 0 : 1
  name        = "${var.project_path}/documentdb-placeholder"
  description = "Placeholder DocumentDB secret for free-plan deployments where DocumentDB is disabled."
  kms_key_id  = var.kms_key_arn
}
