...
resource "aws_vpc_endpoint" "interface" {
  for_each = var.enable_interface_vpc_endpoints ? local.interface_endpoint_services : toset([])
...
resource "aws_networkfirewall_firewall_policy" "main" {
  ...
  firewall_policy {
    ...
    stateful_rule_group_reference {
      priority     = 1
      resource_arn = aws_networkfirewall_rule_group.egress_domain_deny[0].arn
    }
  }
}
...
locals {
  firewall_endpoint_ids_by_az = var.enable_network_firewall ? {
...
resource "aws_route" "private_app_to_firewall" {
  count                  = var.enable_network_firewall ? length(var.private_app_subnet_cidrs) : 0
  route_table_id         = aws_route_table.private_app[count.index].id
...
resource "aws_route" "private_app_to_nat" {
  count                  = var.enable_network_firewall ? 0 : length(var.private_app_subnet_cidrs)
  route_table_id         = aws_route_table.private_app[count.index].id
  destination_cidr_block = "0.0.0.0/0"
  nat_gateway_id         = aws_nat_gateway.main[min(count.index, length(aws_nat_gateway.main) - 1)].id
}
